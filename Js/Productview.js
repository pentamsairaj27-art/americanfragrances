$(document).ready(function () {
    ...
    var Type = "";
    var subcategoryname = "";
    $(document).on('click', '.dupProWidth1', function () {
        // Remove active class from all
        $('.prod_weight').removeClass('lbquanty_kg_active');

        // Add active class to the clicked one
        $(this).find('.prod_weight').addClass('lbquanty_kg_active');

        // Get ID and call your product function
        var getid = $(this).find('span').attr("id");
        $('#product_carosel').trigger('destroy.owl.carousel');
        $('#brand_product_carosel').trigger('destroy.owl.carousel');
        getproduct(getid);
    });
    ...
    function getproduct(getid) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                ProductGender = data.categoryname;
                    ...
                    success: function (data) {
                        ...
                        $.each(data.Products, function (Index, value) {
                            if (getid != value.id) {
                                ...
                                i = i + 1;
                                var customerGender = ProductGender === "Men" ? "Men" : (ProductGender === "Women" ? "Women" : "Unisex");
            ...
            `;
                                            ...
                                            $("#brand_product_carosel #barcode" + product_id).addClass("d-flex flex-row justify-content-center align-items-center").css("margin-top", "28px").append('<div><h5 class="websiteBlue mb-0">Save Big!</h5></div>');
                ...
                if (subcategoryname != null && subcategoryname !== "") {
                    ...
                    var bannerMap = {
                        ...
                        "BATH & BODY": "PRODUCT-PAGE-BANNERS.jpg",
                ...
                if (data.stock != 0) {
                    ...
                    //$(".productViewBelowCon").append(productBlCon);
                    stock = '<span class="text-center" style="color:red; margin-right:20px;">Sold Out</span>';
                    //$(".cart_btn").hide();
                    //$(".attr-heading").hide();
                    //$(".quantity").hide();
                    //$(".addtocart").hide();
                ...
                $("#productview").empty();
                const proid = data.id;
                const prodactive = getid === proid;
                const activesize = prodactive ? "prod_weight lbquanty_kg_active" : "prod_weight lbquanty_kg";
                ...
                var newrowcontent =
                    ...
                    '<img id="recomededImgProduct" src="Images/ameriftagRecommendedIcon.png" style="width:18%!important"/>' +
                    ...
                    '</button></div>' +
                    '</div>' +
                    '<div class="d-none d-md-block d-lg-block pt-5 text-center"><p style="font-size:20px;margin-bottom:0px;">Explore more of</p><p><a href="show-all.html?brand=' + data.brandname + '" style="font-weight:bold; font-size:25px;" class="websiteBlue">' + data.brandname + '</a></p></div>' +
                    '</div>' +
                    ...
                    '<p class="sm-m-0 category_line" style="color: #939292;"><span>Product No: </span>' + data.SKU_ID + '</p>' +
                    '<p class="mt-2 d-none d-md-block d-lg-block category_line">' + data.subcategoryname + '</p>' +
                    '<div class="d-flex d-md-none d-lg-none align-items-center my-1"><div><strike style="color:#838383"><p class="mb-2 category_line" style="color:#838383">$' + originalPrice + '</p></strike></div><div><p class="mb-2 px-3 category_line" style="font-size: 17px!important;">$' + salePrice + '</p></div></div>' +
                    '</div>' +
                    '</div>' +
                    ...
                    '<div class="d-flex flex-row"><div class="stars"></div> <a class="p-1" target="_blank" href ="/reviews.html?id=' + getid + '" class="view_review" > (' + data.rating_count + ' Reviews)</a ></div>' +
                    ...
                    '<div class="col-lg-3 m-3 m-md-0 m-lg-0 quantityCol">' +
                    '<p class="attr-heading">Quantity</p>' +
                    ...
                    '</div></div>' +
                    '</div>' +
                    '<div class="col-lg-9 nofiywed">' +
                    '<div class="d-inline d-md-flex d-lg-flex cart-btns container p-0 gap-3">' +
                    '<div class="mb-2 m-md-0 m-lg-0">' +
                    '<button class="btn wishlist-add mb-4 notifyback" data="' + data.id + '">Notify me</button>' +
                    '<button class="btn btn1 btn-dark cart_btn m-0 addtocart" data="' + data.id + '" type="button">Add to Cart</button>' +
                    ...
                    '<span style="padding-right:10px"><b>Type:</b> ' + data.different_Gender + '</span>' +
                    '<span><b>Stock:</b> ' + data.stock + '</span>' +
                    '</span>' +

                    (data.subcategoryname === "MAKE UP"
                        ? '<span><b>Size:</b> ' + data.dimension + '</span>'
                        : '') +

                    (data.subcategoryname !== "FRAGRANCES" && data.subcategoryname !== "TESTERS" && data.subcategoryname !== "MAKE UP"
                        ? '<span><b>Shade:</b> ' + data.Shade + '</span>'
                        : '') +

                    '<span class="fw-bold"><b>Price:</b> $' + data.price + '.00</span>' +
                    '</span>' +
                    '</button>' +
                    '</span>' +
                    '</div>' +
                    '<div class="row d-flex flex-row justify-content-center" id="duplicate_product"></div>' +
                    ...
                    (subcategoryname === "FRAGRANCES" ?
                        ...
                        '<a type="button" class="btn herewhatGo" data-bs-toggle="modal" data-bs-target="#analyizepopup">Understanding the Amerifrag Barcode</a>' +
                        '</span></div>' : '') +
                    '</div></div>';
                ...
                if (stock_count === 0) {
                    $(".cart_btn, .attr-heading, .quantityCol, .addtocart").hide();
                    $(".notifyback").show();

                    $(".quantityCol").removeClass("col-md-3");
                    $(".nofiywed").removeClass("col-lg-9").addClass("col-lg-12");
                   
                }
                if (stock_count !== 0) {

                    $(".attr-heading, .quantityCol, .addtocart").show();
                    $(".notifyback").hide();

                    $(".quantityCol").addClass("col-lg-3");
                    $(".nofiywed").removeClass("col-lg-12").addClass("col-lg-9");
                    $(".cart-btns").addClass("justify-content-between");
                }
                ...
                if (data.different_value) {
                    ...
                    var dup_products_weight = data.different_value.split(",");
                    var dup_products_array = data.different_id.split(",");
                    var dup_price_array = data.different_price.split(",");
                    var dup_Gender_array = data.different_Gender ? data.different_Gender.split(",") : [];
                    var dup_stock_array = data.different_stock ? data.different_stock.split(",") : [];
                    var dup_shades_array = data.different_Shade ? data.different_Shade.split(",") : [];
                    var dubdimension_array = data.different_dimension ? data.different_dimension.split(",") : [data.dimension];

                    var combinedData = dup_products_weight.map((weight, index) => ({
                        weight: weight,
                        id: dup_products_array[index],
                        price: dup_price_array[index],
                        gender: dup_Gender_array[index] || "",
                        stock: dup_stock_array[index] || "",
                        shade: dup_shades_array[index] || "",
                        dimension: dubdimension_array[index] || ""
                    }));

                    // Sort dimensions low-to-high
                    combinedData.sort((a, b) => parseFloat(a.dimension) - parseFloat(b.dimension));
                    ...
                    if (combinedData.length) {
                        ...
                        var dup_product_header = `
            <h6 class="mb-2 pt-3 px-2 d-flex flex-row justify-content-between selectSize attr-heading">Available Sizes</h6>
            <span class="px-0" id="dup_products"></span>
            <div class="marg_bttm"></div>
        `;
                        $("#duplicate_product").append(dup_product_header);
                        $("#orginal_products").append('<span id="org_product"></span>');
                    }

                    // Render buttons
                    combinedData.forEach((item) => {
                        const dimension = getid;
                        const isActive = item.id === dimension;
                        const class_name = isActive ? "prod_weight lbquanty_kg_active" : "prod_weight lbquanty_kg";
                        var stock = parseFloat(item.stock)
                        const out_of_stock = stock <= 0 ? "out-of-stock" : "";
                        const dupstockText = item.stock > 0 ? "In Stock" : "Sold Out";

                        $("#dup_products").append(`
        <button class="dupProWidth1 ${out_of_stock}" id="${item.id}">
        <a  class="m-0">
            <span id="${item.id}" class="${class_name} d-flex justify-content-between align-items-center">
                <span>${data.subcategoryname === "MAKE UP" ? item.shade : item.dimension}</span>
            </span>
            <span class="RemaingDupInforma">
                <span class="d-flex flex-row justify-content-between">
                ...
                ${data.subcategoryname === "MAKE UP" ? `<span><b> Size:</b> ${item.dimension}</span>` : ""}
               
                <span class="fw-bold"><b>Price: </b>$${item.price}</span>
            </span>
            </a>
        </button>
    `);
                ...
                if (subcategoryname == "FRAGRANCES" || subcategoryname == "TESTERS") {
                    var question1 = {
                        ...
                        Others: "A fragrance that offers a subtly Distinctive aroma that whispers individuality with some intriguing notes"
                    };
                    var question5 = {
                    ...
                    var question10 = {
                        ...
                        Heritage: "Its overall Vintage presentation captures the essence of classic designs and nostalgic charm that transports you to an era of timeless sophistication",
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            var newrow =
                                ...
                                "</div></div></div>";
                            $("#barcodeview" + getid).append(newrow);
                            //  $("#highestValueLabel").text(maxLabel); // Display the highest value label

                            //});
                            var data1 = {
                                question1: response.Products.Labels[0]
                            };
                            //question2 is 5th
                            var data2 = {
                                /*question2: data.question4*/
                                question2: response.Products.Labels[1]
                            };
                            var data3 = {
                                question3: response.Products.Labels[2]
                            };
                            //question4 is 2
                            var data4 = {
                                /* question4: data.question5*/
                                question4: response.Products.Labels[3]
                            };
                            //question5 is 4
                            var data5 = {
                                /* question5: data.question2*/
                                question5: response.Products.Labels[4]
                            };
                            //question6 is 7
                            var data6 = {
                                /* question6: data.question9*/
                                question6: response.Products.Labels[5]
                            };
                            var data7 = {
                                /*question7: data.question6*/
                                question7: response.Products.Labels[6]
                            };
                            var data8 = {
                                /*question8: data.question7*/
                                question8: response.Products.Labels[7]
                            };
                            var data9 = {
                                /*question9: data.question8*/
                                question9: response.Products.Labels[8]
                            };
                            var data10 = {
                                question10: response.Products.Labels[9]
                            };
                ...
                let baseUrl2 = "https://api.americanfragrances.com/Home/FilterProducts";
                let filterObject2 = {
                    customerId: cust_auth || null,
                    subcategory: SubcatForSimilar,
                    style: style,
                ...
                // Clean object
                Object.keys(filterObject2).forEach(key => {
                    if (filterObject2[key] === undefined || filterObject2[key] === null) {
                        delete filterObject2[key];
                    }
                });
                var ajaxData2 = filterObject2;
                // Construct final URL
            ...
            `;
                                            ...
                                            $("#barcode" + product_id).addClass("d-flex flex-row justify-content-center align-items-center").css("margin-top", "28px").append('<div><h5 class="websiteBlue mb-0">Save Big!</h5></div>');
                        ...
                        $("#product_carosel").owlCarousel({
                            ...
                            $("#product_carosel").empty();
                            $("#product_carosel").hide();
                            $("#productCuroselNoProductsDiv").empty();
                            // Optional: show a friendly message when there are no products
